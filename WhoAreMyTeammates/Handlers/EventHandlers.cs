...
namespace WhoAreMyTeammates.Handlers
{
    ...
    {
        public readonly WhoAreMyTeammates plugin;
                ...
                if (scpCount == 1)
            {
                if (plugin.Config.IsBCDelayEnabled == true)
                    Timing.CallDelayed(plugin.Config.WamtBCDelay, () =>
                    {
                        ...
                        {
                            scp.Broadcast(plugin.Config.WamtBCTime, plugin.Config.SingleM);
                            Log.Debug($"Successfully sent bc to player - Only one SCP In Game.", plugin.Config.EnableDebug);
                        }
                    });
                else
                {
                    ...
                    if (Exiled.Loader.Loader.Plugins.Any(p => p.Name == "ScpSwap" && p.Config.IsEnabled))
                    {
                        ...
                        Timing.CallDelayed(plugin.Config.WamtBCDelay, () =>
                        {
                            foreach (var scp in scps) ///Sends Broadcast to every player in the list of "scps"
                            {
                                ...
                                Log.Debug($"Successfully sent bc to players. The following SCPs are in game: {scpinfos}.", plugin.Config.EnableDebug);
                            }
                        });
                    }
                    else
